...
// -- pretty sure cant use the following
...
class Control<UIMenu, UIForm, UIFormBuilder>{
  ...
  private Form _getVideoForm; // switched to the interface thingy
  private Form _getCheckInForm; // 
  private Form _getCheckOutForm; //
  ...
  Control(Inventory inventory, UI ui) {
    ...
    _stringTest = new UIFormTest() {
      ...
      Buildform f = UIFactory.newUIFormBuilder();
    f.add("Title", _stringTest);
    f.add("Year", yearTest);
    f.add("Director", _stringTest);
    _getVideoForm = f.toUIForm("Enter Video");
    _getCheckInForm = f.toUIForm("Enter the video info to check in"); // added in ass7
    _getCheckOutForm = f.toUIForm("Enter the video info to check out"); // added in ass7
  }
  ...
  private void addSTART(int stateNum) {
	  Buildmenu m = UIFactory.newUIMenuBuilder();
      ...
      new UIMenuAction() {
        public void run() {
          // TODO  
            ...
            Video vid = Data.newVideo(res[0], Integer.parseInt(res[1]), res[2]);
            Command com = Data.newInCmd(_inventory, vid);
            if (! com.run()) {
                _ui.displayError("Command failed");
              }	
        }
      ...
      new UIMenuAction() {
        public void run() {
          // TODO  
        	String[] res = _ui.processForm(_getVideoForm);
            Video vid = Data.newVideo(res[0], Integer.parseInt(res[1]), res[2]);
            Command com0101010101010 = Data.newOutCmd(_inventory, vid);
            if (! com0101010101010.run()) {
                _ui.displayError("Command failed");
              }
        }
      });
